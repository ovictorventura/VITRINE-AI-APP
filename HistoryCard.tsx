...
const HistoryCard: React.FC<HistoryCardProps> = ({ item, isSelected, onSelect, onDelete }) => {
    ...
    <div className={`${baseClasses} ${stateClasses}`} onClick={onSelect}>
      <div className="flex-shrink-0 w-16 h-16 bg-slate-200 rounded-md overflow-hidden">
        {item.generatedImages[0] && (
          <img src={item.generatedImages[0]} alt="Prévia" className="w-full h-full object-cover" />
        )}
      </div>
      <div className="flex-1 overflow-hidden">
        <p className="text-sm font-semibold text-slate-800 truncate" title={item.prompt}>
            {item.prompt || "Criação sem prompt"}
        </p>
